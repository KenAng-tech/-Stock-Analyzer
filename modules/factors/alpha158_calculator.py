...
class Alpha158Calculator:
    ...
    def calculate_all(self, klines: List[Dict], stock_code: str = '',
        ...
        """
        计算所有 Alpha158 因子 (P1-5 语义缓存旁路, 默认关)

        Args:
            klines: K 线数据 [{date, open, high, low, close, volume, ...}]
            stock_code: 代码 (use_cache=True 时作缓存键一部分)
            use_cache: True 时经 factor_cache 表达式语义缓存 (命中免重算,
                       右端落后只补尾部); 默认 False = 现有行为零变化

        Returns:
            {factor_name: np.ndarray} 因子矩阵
        """
        if use_cache and stock_code and klines:
            try:
                ...
                cached = factor_cache.get_or_compute(
                    'alpha158_full', stock_code, klines, _compute, lookback=60)
                return {k: np.asarray(v, dtype=np.float64) for k, v in cached.items()}
            except Exception as e:
                logger.error(f"[Alpha158] 语义缓存旁路失败, 直接计算: {e}", exc_info=True)
        return self._calculate_all_uncached(klines)

    def _calculate_all_uncached(self, klines: List[Dict]) -> Dict[str, np.ndarray]:
        """
        全量计算所有 Alpha158 因子 (缓存关闭/回源路径, 原实现)
        ...
        # 使用局部集合记录本次计算的因子名，避免重复追加
        added_names = set(self.factor_names)
        ...
        if len(returns) > 20:
            ...
            for n in ('upside_volatility', 'downside_volatility', 'up_down_vol_ratio'):
                if n not in added_names:
                    self.factor_names.append(n)
                    added_names.add(n)
        ...
        for n in ('macd', 'macd_signal', 'macd_hist'):
            if n not in added_names:
                self.factor_names.append(n)
                added_names.add(n)
        ...
        if 'boll_position' not in added_names:
            self.factor_names.append('boll_position')
            added_names.add('boll_position')
        ...
        if len(closes) > 20:
            ...
            for n in ('return_skewness', 'return_kurtosis'):
                if n not in added_names:
                    self.factor_names.append(n)
                    added_names.add(n)
        ...
        if len(returns) > 30:
            ...
            if len(ret_short) == len(ret_long) and len(ret_short) > 5:
                ...
                factors['return_autocorr'] = 0
            if 'return_autocorr' not in added_names:
                self.factor_names.append('return_autocorr')
                added_names.add('return_autocorr')
    ...
    def reset_factor_names(self):
        """重置因子名称列表 (用于重新计算)"""
        self.factor_names = []
        self._build_factor_names()

    def select_top_factors(self, factors: Dict[str, np.ndarray],
                           returns: List[float], n: int = 20) -> List[str]:
        """
        根据 IC 绝对值选择 Top N 因子

        Args:
            factors: {factor_name: np.ndarray} 因子矩阵
            returns: 收益率序列
            n: 选择因子数量
        ...
        for name, factor_values in factors.items():
            # 兼容 scalar (np.float64) 和 array (np.ndarray)
            if isinstance(factor_values, np.ndarray) and len(factor_values) != len(returns):
                continue
            try:
                # 计算因子与收益的 Spearman 秩相关系数
                if isinstance(factor_values, np.ndarray):
                    ic = np.corrcoef(factor_values[-len(returns):], np.array(returns))[0, 1]
                else:
                    # scalar: 用单点计算
                    ic = 0.0
                if not np.isnan(ic):
